...
model_fold = './model';
fine_tune_type = 'ft_weight_sum_10_0.2_rand_low_lr_triplet_0.1_4gpu';
start_epoch = 1;
opts.training_method = 'triplet';
...
for epoch_num =start_epoch: max_epoch_num
    ...
    for bthid = 1:opts.batch_num_per_epoch
        ...
        batch = gen_batch_triplet(solver.net,imdb,bthid,opts);
        ...
        solver.step(1);
        iter_ = iter_ + 1;
%         avg_a = solver.net.blob_vec(solver.net.name2blob_index('avg_a'));
%         avg_a_diff = avg_a.get_diff();avg_a_diff = squeeze(avg_a_diff);
%         avg_p = solver.net.blob_vec(solver.net.name2blob_index('avg_p'));
%         avg_p_diff = avg_p.get_diff();avg_p_diff = squeeze(avg_p_diff);
%         avg_n = solver.net.blob_vec(solver.net.name2blob_index('avg_n'));
%         avg_n_diff = avg_n.get_diff();avg_n_diff = squeeze(avg_n_diff);
    ...
    for val_iter = 1:val_num
        test_batch = gen_batch_triplet(solver.net,imdb,val_iter,opts);
