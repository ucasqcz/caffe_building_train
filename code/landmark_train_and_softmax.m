...
model_fold = './model/new_experiment/softmax';
fine_tune_type = 'ft_weight_sum_b1_720_without_pca_landmark_0.0005';
start_epoch = 1;
...
%         batch = gen_batch(solver.net,imdb,bthid,batchsize,neg_num,'train');
        ...
        train_s_loss = solver.net.blob_vec(solver.net.name2blob_index('softmax_loss'));
        ...
        if(mod(iter_ - 1,show_per_iter) == 0)
            fprintf('iter %d --- train loss: %f  softmaxloss: %f\n',iter_,mean_train_loss,mean_train_s_loss);
        end
    end
    ...
    for val_iter = 1:val_num
        ...
        test_s_loss = solver.net.blob_vec(solver.net.name2blob_index('softmax_loss'));
